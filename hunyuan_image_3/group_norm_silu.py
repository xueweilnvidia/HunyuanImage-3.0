"""Fused GroupNorm + SiLU for HunyuanImage VAE inference.

Adapted from SGLang's diffusion Triton GroupNorm+SiLU kernel. This module is
self-contained and falls back to PyTorch when Triton or the input constraints do
not fit.
"""
...
_USE_TRITON = os.environ.get("HUNYUAN_USE_TRITON_GROUP_NORM_SILU", "1") != "0"
...
if triton is not None:
    ...
    def _group_norm_finalize_stats_kernel(
        ...
        for off in range(0, chunks_per_row, BLOCK_SIZE):
            ...
            sum_val += tl.sum(tl.load(partial_sum_ptr + base + idx, mask=mask, other=0.0), axis=0)
            sum_sq += tl.sum(tl.load(partial_sq_ptr + base + idx, mask=mask, other=0.0), axis=0)
...
def _launch_chunked(
    ...
    y_flat = y.reshape(-1)
    partial_sum = torch.empty((rows, chunks_per_row), device=x_contiguous.device, dtype=torch.float32)
...
def triton_group_norm_silu(
    ...
    if not _can_use_triton_group_norm_silu(x, weight, bias, num_groups):
        return group_norm_silu_native(x, weight, bias, num_groups, eps)
...
def apply_group_norm_silu(x: torch.Tensor, norm: nn.GroupNorm) -> torch.Tensor:
    if not isinstance(norm, nn.GroupNorm) or not norm.affine:
        return F.silu(norm(x))
    return triton_group_norm_silu(x, norm.weight, norm.bias, norm.num_groups, norm.eps)


__all__ = ["apply_group_norm_silu", "triton_group_norm_silu"]
